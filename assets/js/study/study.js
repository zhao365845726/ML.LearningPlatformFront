	...
	$(document).ready(function() {
					...
					success: function(data, type) {
						if(data.status_code == 200){
							...
							if(title == '在线学习'){
								...
								window.location.href = '/compoents/study/studyLogin.html?title='+title;
							}
						}
					}
				})
		...
		var everydayquestionlist = function(){
			...
			$.ajax({
				...
				success: function(data, type) {
					if(data.data) {
						data.data.lst_everydayquestion && $.each(data.data.lst_everydayquestion, function (index, item) {
...
function isFinger(obj){
	if(IsPC()){
		...
		if(status == 1){
			...
			window.localStorage.removeItem("remainingTime"+$.cookie('userId')+UserTPLibId); 
		}
